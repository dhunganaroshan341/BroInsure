<?php

namespace App\Http\Controllers\Admin;

use App\Http\Controllers\BaseController;
use App\Http\Requests\GroupPackageDeleteRequest;
use App\Http\Requests\GroupPackageStoreRequest;
use App\Http\Requests\GroupStoreRequest;
use App\Http\Requests\GroupUpdateRequest;
use App\Models\Client;
use App\Models\CompanyPolicy;
use App\Models\Group;
use App\Models\GroupHeading;
use App\Models\GroupPackage;
use App\Models\GroupPackageHeadings;
use App\Models\InsuranceHeading;
use App\Models\Package;
use Illuminate\Http\Request;
use DataTables;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Log;

class GroupController extends BaseController
{

    public function index(Request $request)
    {

        $access = $this->accessCheck('group');
        if ($request->ajax()) {
            $users = Group::select('*')->whereHas('policy', function ($q) {
                $q->where('policy_type', 'group');
            })->with([
                'client' => function ($query) {
                    $query->withoutGlobalScopes();
                }
            ])->orderBy('id', 'asc');
            return Datatables::of($users)
                ->addIndexColumn()
                ->editColumn('client_id', function ($row) {
                    return $row->client->name;
                })
                ->addColumn('action', function ($row) use ($access) {

                    $btn = '';

                    $btn .= "<a href='javascript:void(0)' class='btn btn-warning btn-sm viewData mx-1' data-pid='" . $row->id . "' data-url=" . route('groups.edit', "$row->id") . "><i class='fas fa-eye'></i> View</a>";
                    if ($access['isedit'] == 'Y' && $row->id != 1) {
                        $btn .= "<a href='javascript:void(0)' class='edit btn btn-primary btn-sm editData' data-pid='" . $row->id . "' data-url=" . route('groups.edit', "$row->id") . "><i class='fas fa-edit'></i> Edit</a>";
                    }
                    if ($access['isdelete'] == 'Y' && $row->id != 1) {
                        $btn .= "&nbsp;<a href='javascript:void(0)' class='edit btn btn-danger btn-sm deleteData' data-pid='" . $row->id . "' data-url=" . route('groups.destroy', "$row->id") . "><i class='fas fa-trash'></i> Delete</a>";
                    }

                    return $btn;
                })
                ->rawColumns(['action'])
                ->make(true);
        }
        $data['access'] = $access;
        $data['title'] = 'insurance Goups';
        $data['extraCss'] = commonDatatableFiles('css');
        $data['extraJs'] = commonDatatableFiles();
        $data['clients'] = Client::get(['id', 'name']);
        $data['extraCss'][] = 'admin/cdns/select2-bootstrap-5/dist/select2-bootstrap-5-theme.min.css';
        $data['extraCss'][] = 'admin/cdns/select2@4.1/dist/css/select2.min.css';
        $data['extraJs'][] = 'admin/cdns/select2@4.1/dist/js/select2.min.js';
        // $data['packages'] = Package::get(['id', 'name']);
        $data['headings'] = InsuranceHeading::with(['sub_headings'])->where('status', 'Y')->orderBy('name', 'asc')->get(['id', 'name']);
        return view('backend.group.list', $data);
    }

    /**
     * Show the form for creating a new resource.
     */
    public function create()
    {
        //
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(GroupStoreRequest $request)
    {
        $accessCheck = $this->checkAccess($this->accessCheck('group'), 'isinsert');
        if ($accessCheck && $accessCheck['status'] == false) {
            return response()->json(['status' => $accessCheck['status'], 'message' => $accessCheck['message'], 403]);
        }
        try {
            DB::beginTransaction();
            $validatedData = $request->validated();
            // dd( array_key_exists('is_amount_different', $validatedData));
            $groupData = [
                'client_id' => $validatedData['client_id'],
                'insurance_amount' => $validatedData['insurance_amount'],
                'policy_id' => $validatedData['policy_id'],
                'name' => $validatedData['name'],
                'code' => $validatedData['code'],
                'is_amount_different' => array_key_exists('is_amount_different', $validatedData) ? 'Y' : 'N',
                'is_imitation_days_different' => $validatedData['is_imitation_days_different'],
            ];
            $group = Group::create($groupData);
            $includingSubHeadings = $validatedData['sub_heading_id'];
            if ($validatedData['is_imitation_days_different'] !== 'Y') {
                $imitation_days = CompanyPolicy::where('id', $validatedData['policy_id'])->value('imitation_days');
            }
            foreach ($validatedData['heading_id'] as $key => $heading_id) {
                $heading = InsuranceHeading::where('id', $heading_id)->with('sub_headings')->first();
                $current_imitation_days = $validatedData['is_imitation_days_different'] === 'Y'
                    ? $validatedData['imitation_days'][$key]
                    : $imitation_days;
                $groupHeadingData = [
                    'group_id' => $group->id,
                    'heading_id' => $heading_id,
                    'is_employee' => isset($validatedData['is_employee'][$key]) ? 'Y' : 'N',
                    'is_spouse' => isset($validatedData['is_spouse'][$key]) ? 'Y' : 'N',
                    'is_child' => isset($validatedData['is_child'][$key]) ? 'Y' : 'N',
                    'is_parent' => isset($validatedData['is_parent'][$key]) ? 'Y' : 'N',
                    'amount' => $validatedData['amountNew'][$key],
                    'imitation_days' => $current_imitation_days,
                    'is_spouse_amount' => array_key_exists('is_amount_different', $validatedData) ? $validatedData['is_spouse_amount'][$key] : null,
                    'is_child_amount' => array_key_exists('is_amount_different', $validatedData) ? $validatedData['is_child_amount'][$key] : null,
                    'is_parent_amount' => array_key_exists('is_amount_different', $validatedData) ? $validatedData['is_parent_amount'][$key] : null,
                ];
                if ($heading) {
                    $assignedHeading = $heading->sub_headings->pluck('id')->toArray();
                    $commonValues = array_intersect($assignedHeading, $includingSubHeadings);
                    // Ensure it is a numerically indexed array
                    $commonValues = array_values($commonValues);
                    // Encode to JSON
                    $jsondata = json_encode($commonValues);
                    $groupHeadingData['exclusive'] = $jsondata;
                }
                GroupHeading::create($groupHeadingData);
            }
            DB::commit();
            return $this->sendResponse(true, getMessageText('insert'));
        } catch (\Exception $e) {
            DB::rollback();
            Log::error($e);
            return $this->sendError(getMessageText('insert', false));
        }
    }

    /**
     * Display the specified resource.
     */
    public function show(Group $group)
    {
        //
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit($id)
    {
        $accessCheck = $this->checkAccess($this->accessCheck('group'), 'isedit');
        if ($accessCheck && $accessCheck['status'] == false) {
            return response()->json(['status' => $accessCheck['status'], 'message' => $accessCheck['message'], 403]);
        }
        $group = Group::with(['headings', 'client'])->find($id);
        if ($group) {
            return $this->sendResponse($group, getMessageText('fetch'));
        } else {
            return $this->sendError(getMessageText('fetch', false));
        }
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(GroupUpdateRequest $request, $id)
    {
        // dd($request->all());
        $accessCheck = $this->checkAccess($this->accessCheck('group'), 'isupdate');
        if ($accessCheck && $accessCheck['status'] == false) {
            return response()->json(['status' => $accessCheck['status'], 'message' => $accessCheck['message'], 403]);
        }
        try {
            DB::beginTransaction();
            $validatedData = $request->validated();

            $group = Group::with(['headings'])->find($id);
            $groupData = [
                'client_id' => $validatedData['client_id'],
                'insurance_amount' => $validatedData['insurance_amount'],
                'name' => $validatedData['name'],
                'code' => $validatedData['code'],
                'is_amount_different' => array_key_exists('is_amount_different', $validatedData) ? 'Y' : 'N',
                'is_imitation_days_different' => $validatedData['is_imitation_days_different'],
            ];
            $group->update($groupData);
            $includingSubHeadings = $validatedData['sub_heading_id'];
            if ($validatedData['is_imitation_days_different'] !== 'Y') {
                $imitation_days = CompanyPolicy::where('id', $validatedData['policy_id'])->value('imitation_days');
            }
            GroupHeading::where('group_id', $id)->whereNotIn('heading_id', $validatedData['heading_id'])->delete();
            foreach ($validatedData['heading_id'] as $key => $heading_id) {
                $heading = InsuranceHeading::where('id', $heading_id)->with('sub_headings')->first();
                $current_imitation_days = $validatedData['is_imitation_days_different'] === 'Y'
                    ? $validatedData['imitation_days'][$key]
                    : $imitation_days;
                $groupHeadingData = [
                    'is_employee' => isset($validatedData['is_employee'][$key]) ? 'Y' : 'N',
                    'is_spouse' => isset($validatedData['is_spouse'][$key]) ? 'Y' : 'N',
                    'is_child' => isset($validatedData['is_child'][$key]) ? 'Y' : 'N',
                    'is_parent' => isset($validatedData['is_parent'][$key]) ? 'Y' : 'N',
                    'amount' => $validatedData['amountNew'][$key],
                    'imitation_days' => $current_imitation_days,
                    'is_spouse_amount' => array_key_exists('is_amount_different', $validatedData) ? $validatedData['is_spouse_amount'][$key] : null,
                    'is_child_amount' => array_key_exists('is_amount_different', $validatedData) ? $validatedData['is_child_amount'][$key] : null,
                    'is_parent_amount' => array_key_exists('is_amount_different', $validatedData) ? $validatedData['is_parent_amount'][$key] : null,
                ];
                if ($heading) {
                    $assignedHeading = $heading->sub_headings->pluck('id')->toArray();
                    $commonValues = array_intersect($assignedHeading, $includingSubHeadings);
                    // Ensure it is a numerically indexed array
                    $commonValues = array_values($commonValues);
                    // Encode to JSON
                    $jsondata = json_encode($commonValues);
                    $groupHeadingData['exclusive'] = $jsondata;
                }
                GroupHeading::updateOrCreate([
                    'group_id' => $id,
                    'heading_id' => $heading_id
                ], $groupHeadingData);
            }
            DB::commit();
            return $this->sendResponse(true, getMessageText('update'));
        } catch (\Exception $e) {
            DB::rollback();
            Log::error($e);
            return $this->sendError(getMessageText('update', false));
        }
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy($id)
    {
        $accessCheck = $this->checkAccess($this->accessCheck('group'), 'isdelete');
        if ($accessCheck && $accessCheck['status'] == false) {
            return response()->json(['status' => $accessCheck['status'], 'message' => $accessCheck['message'], 403]);
        }
        $group = Group::find($id);
        if (!$group) {
            return $this->sendError(getMessageText('delete', false));
        }
        $isdel = $group->delete();
        if ($isdel) {
            return $this->sendResponse(true, getMessageText('delete'));
        } else {
            return $this->sendError(getMessageText('delete', false));
        }
    }
    public function package_headings(Request $request, $id)
    {
        $package = Package::with(['headings'])->find($id);
        // $grouppackage=GroupPackage::where('group_id',$request->group_id)->where('package_id',$id)->first();

        $checklist = GroupPackageHeadings::whereHas('group_package', function ($q) use ($id, $request) {
            $q->where('group_id', $request->group_id)->where('package_id', $id);
        })->get();
        // dd($checklist,$package->headings);
        $html = view('backend.group.package_heading_table', compact('package', 'checklist'))->render();
        return json_encode([
            'status' => 'success',
            'message' => 'Headings Fetched',
            'response' =>
            ['packages' => $package->headings],
            'html' => $html
        ]);
    }
    public function package_get($id)
    {
        $group = Group::with(['packages.package'])->find($id);

        if (isset($group)) {
            $data = $group->packages->map(function ($package) {
                return [
                    'id' => $package->id,
                    'group_id' => $package->group_id,
                    'package_id' => $package->package_id,
                    'total_amount' => $package->total_amount,
                    'package_name' => $package->package->name,
                ];
            });
            // dd($data);
            return $this->sendResponse($data, getMessageText('fetch'));
        } else {
            return $this->sendError(getMessageText('fetch', false));
        }
    }
    public function package_post(GroupPackageStoreRequest $request)
    {
        $accessCheck = $this->checkAccess($this->accessCheck('group_package'), 'isinsert');
        if ($accessCheck && $accessCheck['status'] == false) {
            return response()->json(['status' => $accessCheck['status'], 'message' => $accessCheck['message'], 403]);
        }
        try {
            DB::beginTransaction();
            $validatedData = $request->validated();
            $groupPackage = GroupPackage::updateOrCreate(
                [
                    'group_id' => $validatedData['group_id'],
                    'package_id' => $validatedData['package_id'],
                ],
                ['total_amount' => array_sum($validatedData['amount'])]
            );
            $groupPackage->headings()->delete();
            foreach ($validatedData['heading_id'] as $key => $heading_id) {
                GroupPackageHeadings::create([
                    'group_package_id' => $groupPackage->id,
                    'heading_id' => $heading_id,
                    'is_employee' => isset($validatedData['is_employee'][$key]) ? 'Y' : 'N',
                    'is_spouse' => isset($validatedData['is_spouse'][$key]) ? 'Y' : 'N',
                    'is_child' => isset($validatedData['is_child'][$key]) ? 'Y' : 'N',
                    'is_parent' => isset($validatedData['is_parent'][$key]) ? 'Y' : 'N',
                    'amount' => $validatedData['amount'][$key],
                ]);
            }

            DB::commit();
            return $this->sendResponse(true, getMessageText('insert'));
        } catch (\Exception $e) {
            DB::rollback();
            Log::error($e);
            return $this->sendError(getMessageText('insert', false));
        }
    }

    public function package_delete(GroupPackageDeleteRequest $request)
    {
        $accessCheck = $this->checkAccess($this->accessCheck('group_package'), 'isdelete');
        if ($accessCheck && $accessCheck['status'] == false) {
            return response()->json(['status' => $accessCheck['status'], 'message' => $accessCheck['message'], 403]);
        }
        try {
            DB::beginTransaction();
            $groupPackage = GroupPackage::where('package_id', $request->package_id)->where('group_id', $request->group_id)->first();
            if ($groupPackage) {
                $groupPackage->headings()->delete();
                $groupPackage->delete();
            }

            DB::commit();
            return $this->sendResponse(true, getMessageText('insert'));
        } catch (\Exception $e) {
            DB::rollback();
            Log::error($e);
            return $this->sendError(getMessageText('delete', false));
        }
    }
}
