...
function storeMember(element) {
    ...
    postdata.append("member_type", "individual");
...
function populateFieldsForEdit(member) {
    ...
    let auto_populate_input = {
        ...
        policy_id: member?.current_member_policy?.policy_id,
        start_date: member?.current_member_policy?.start_date,
    };
    if(member?.current_member_policy?.policy_id) {
        populateGroups(member?.current_member_policy?.policy_id, member?.current_member_policy?.group_id);
    }
...
$(document).on("click", ".addBtn", function () {
    var newRow = `<div class="newEntry row">
            ...
            <div class="col-md-3">
                <label for="input3" class="form-label">AGE <span class="text-danger">*</span> </label>
                <input type="date" class="form-control"  placeholder="Enter AGE" name="rel_dob[]"
                required>
            </div>
            <div class="col-md-2">
...
$(document).on("change", "#policy_id", function () {
    let id = $("#policy_id").find(":selected").data("id");
    if (id) {
        populateGroups(
            id,
...
function populateGroups(id, selected = null) {
    ...
    let dataurl = "/admin/get-policy-group/" + id;
    var request = getRequest(dataurl);
    request.done(function (res) {
        if (res.status === true) {
...
$(document).on("click", ".viewData", function (e) {
    ...
    request.done(function (response) {
        if (response.status === true) {
            ...
            $("#view_middle_name").html(response.response.personal.middle_name);
            ...
            $("#view_designation").html(response.response.address.designation);
            $("#view_providence").html(response.response.address.perm_province);
            $("#view_district").html(response.response.address.perm_district);
            $("#view_city").html(response.response.address.perm_city);
            $("#view_perm_ward_no").html(
                response.response.address.perm_ward_no
            );
            $("#view_perm_street").html(response.response.address.perm_street);
            $("#view_perm_house_no").html(
                response.response.address.perm_house_no
            );
            $("#view_present_ward_no").html(
                response.response.address.present_ward_no
            );
            $("#view_present_street").html(
                response.response.address.present_street
            );
            $("#view_present_house_no").html(
                response.response.address.present_house_no
            );
            $("#view_is_address_same").html(
                response.response.address.is_address_same == "Y" ? "Yes" : "No"
            );

            if (response.response.address.is_address_same == "Y") {
                $(".is_address_sameDiv").attr("hidden", true);
            } else {
                $(".is_address_sameDiv").attr("hidden", false);
            }
            $("#view_citizenship_no").html(
                capitalizeFirstLetter(response.response.details.citizenship_no)
            );
            $("#view_citizenship_district").html(
                response.response.details.issued_district
            );
            $("#view_citizenship_issued_date").html(
                response.response.details.citizenship_issued_date
            );
            $("#view_occupation").html(response.response.details.occupation);
            if (response.response.details.occupation === "Other") {
                $("#view_other_occupation").attr("hidden", false);
                $("#view_other_occupation").html(
                    response.response.details.occupation_other
                );
            } else {
                $("#view_other_occupation").attr("hidden", true);
            }
            ...
            // $('#view_type').html(response.response.policy.type);
            $(".onlyGroup").attr("hidden", true);
